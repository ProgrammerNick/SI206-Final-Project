...
def daily_high_and_low(cur, day, city):
    cur.execute("SELECT id from Cities WHERE city = ?", (city,))
    city_id = int(cur.fetchone()[0])
    cur.execute("SELECT MIN(temperature) from Weather WHERE city_id = ? AND date = ?", (city_id, day))
    min_temp = int(cur.fetchone()[0])
    cur.execute("SELECT MAX(temperature) from Weather WHERE city_id = ? AND date = ?", (city_id, day))
...
def daily_feels_like_data(cur, day, city):
    pass
...
def main():
    ...
    create_weather_table(weather_dict, cur, conn, 1)
    print(daily_avg(cur, 20250415, "Ann Arbor"))
    print(daily_high_and_low(cur, 20250415, "Ann Arbor"))
